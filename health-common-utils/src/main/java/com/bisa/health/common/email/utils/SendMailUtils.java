...
public class SendMailUtils {
	...
	public boolean sendCodeByMail(String username, String code) {
		...
		boolean result = mu.sendMailBySSL(MailContent.ALIDM_SMTP_HOST, MailContent.ALIDM_SMTP_PORT, MailContent.FROM,
				MailContent.PASSWORD, username, subject, MailContent.sentVarifyCodeMail(code));
		if (!result) {
			return false;
		}
		return true;
	...
	public boolean sendUrlByMail(String username, String varify_url) {
		...
		MailUtil mu = new MailUtil();
		boolean result = mu.sendMailBySSL(MailContent.ALIDM_SMTP_HOST, MailContent.ALIDM_SMTP_PORT, MailContent.FROM,
				MailContent.PASSWORD, username, subject, MailContent.sentVarifyUrlMail(varify_url));
		if (!result) {
			return false;
		}
		return true;
	...
	public boolean sendAlarmByMail(String name, String mail, String time) {
		...
		MailUtil mu = new MailUtil();
		boolean result = mu.sendMailBySSL(MailContent.ALIDM_SMTP_HOST, MailContent.ALIDM_SMTP_PORT, MailContent.FROM,
				MailContent.PASSWORD, mail, subject, MailContent.sentAlarmContentByMail(name, time));
		if (!result) {
			return false;
		}
		return true;
	...
	public boolean sendAlarmByContent(String mail, String content) {
		...
		MailUtil mu = new MailUtil();
		boolean result = mu.sendMailBySSL(MailContent.ALIDM_SMTP_HOST, MailContent.ALIDM_SMTP_PORT, MailContent.FROM,
				MailContent.PASSWORD, mail, subject, content);
		if (!result) {
			return false;
		}
		return true;
	...
	public boolean sendPassword(String username, String six_password) {
		...
		MailUtil mu = new MailUtil();
		boolean result = mu.sendMailBySSL(MailContent.ALIDM_SMTP_HOST, MailContent.ALIDM_SMTP_PORT, MailContent.FROM,
				MailContent.PASSWORD, username, subject, MailContent.sentPasswordMail(six_password));
		if (!result) {
			return false;
		}
		return true;
